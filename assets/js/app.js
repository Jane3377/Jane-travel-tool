/* 貞選旅管家 v66.1 app.js：由穩定版 HTML 內嵌腳本拆出，保留原執行順序與全域函式。 */
...
/* ===== v9 UX overrides: city presets, compact map buttons, flight-to-itinerary, visible photo book style ===== */
const cityMap={
  "韓國":["釜山","首爾","濟州","大邱","仁川"],
...
/* ===== v10 refinements: destination logic, airport presets, hotel itinerary, AI prompt/import ===== */
const cityMapV10={
  "韓國":["釜山","首爾","濟州","大邱","仁川"],
  "日本":["東京","大阪","京都","福岡","札幌","沖繩","名古屋"],
  "泰國":["曼谷","清邁","普吉"],
...
/* ===== v10.1 AI itinerary prompt + import, remove sample from side calendar ===== */
function renderSide(){
...
/* ===== v11: simplified destination, spot AI import, clear plans, mobile calendar UX ===== */
function countryOptions(){
  return Object.keys(cityMapV10).map(c=>`<option value="${c}" ${data.trip.country==c?"selected":""}>${c==="歐洲"?"歐洲區域":c}</option>`).join("")+
    `<option value="其他" ${data.trip.country=="其他"?"selected":""}>其他</option>`;
}
...
/* ===== v14 stable overrides based on uploaded v11 file ===== */
const cityMapFinal={
  "韓國":["釜山","首爾","濟州","大邱","仁川"],
  "日本":["東京","大阪","京都","福岡","札幌","沖繩","名古屋"],
  "香港":["香港"],
...
/* ===== v15 overrides: card collapsed, currency label, mobile top tabs, compact forms, AI spot modal ===== */
...
/* ===== v16 overrides: calendar only in planner, flight auto-add option, hotel auto itinerary, spot link state ===== */
...
/* ===== v18 overrides: print photo book, silent plan delete, guarded country change, hotel date/sort ===== */
...
/* ===== v19 overrides: date reset guard, hotel optional actions, budget toggles, trip cover, refined PDF ===== */
...
/* ===== v21 overrides: country reset on save, option modals, locked names, budget exclusion ===== */
...
/* ===== v22 Firebase single-user cross-device sync ===== */
...
/* ===== v23 validation layer: flight / hotel / plan / imported spots / free budget date ===== */
...
/* ===== v24 overrides: flight import status/actions, smaller sync copy, friendlier labels ===== */
...
/* ===== v25 overrides: account avatar menu, mini sync status, round-trip flight budget ===== */
...
/* ===== v26 overrides: remove plan money/payer, independent budget, account sync state ===== */
let syncStatus = "idle";
...
/* ===== v27 type stability + currentDay UX; no new id/source structure ===== */
...
/* ===== v28 data structure refactor: plan id/source + flight/hotel upsert sync ===== */
/*
新行程資料結構：
{
...
/* ===== v28 reference examples for future maintenance =====
新的資料結構：
plan = {
  id: "flight-out-plane",
  source: "flight",
  sourceType: "flight", // 舊 UI 相容
  day: "2026-06-04",
  start: "16:00",
  end: "19:00",
  type: "航班",
...
刪除流程：
v28DeletePlansBySource("flight")
v28DeletePlansBySource("hotel", p => p.hotelId == hotelId)
*/


/* ===== v29 fixed: flight transfer support without inline script tag ===== */
/*
新資料結構：
...
/* ===== v30 overrides: budget cards + dense collapsible flight segments ===== */
...
/* ===== v31 airport and terminal options for flight segments ===== */
...
/* ===== v32 overrides: compact flight copy + flexible flight date range ===== */
...
/* ===== v33 budget details: desktop table + aligned mobile cards ===== */
function budgetAllItems(){
  const items=[];
  (data.expenses||[]).forEach(x=>{
    items.push({
...
/* ===== v34 targeted fixes: source plan budget skip, no delete confirm, China cities ===== */
...
/* ===== v36 fix flight-to-plan date/time + cleaner direct flight labels ===== */
/* 只修航班帶入行程邏輯，不改資料結構 */
...
/* ===== v38 mobile-only flight UI helpers; data structure unchanged ===== */
...
/* ===== v39 flight save-first workflow + flight plan time rules ===== */
/* 只調整航班與機場接送區塊，不改資料結構 */
...
/* ===== v41 override mobile flight datetime markup only ===== */
function flightSegmentForm(k, idx, seg){
  seg = seg || {};
  const defaultOpen = idx===0 ? "open" : "";
  const fromId=`${k}s${idx}from`;
...
/* ===== v43 photo travel book story redesign only ===== */
function exportPhotoBookPDF(){
...
/* ===== v44 photo book PDF export hint only ===== */
function exportPhotoBookPDF(){
...
/* ===== v45 photo book PDF preview mode only ===== */
...
/* ===== v46 theme switcher: UI-only settings, no travel data structure changes ===== */
/*
theme config 範例：
const THEME_CONFIG = {
  modes: {
...
/* ===== v48 brand wording + boutique PDF export hint only ===== */
function exportPhotoBookPDF(){
...
/* ===== v49 photo book direct story upload only ===== */
...
/* ===== v50 photo book story-only render/export ===== */
function exportPhotoBookPDF(){
...
/* ===== v51-map OpenStreetMap / Leaflet itinerary route card ===== */
...
/* 只覆寫行程頁 renderPlanner，插入地圖卡；不更動 data 結構 */

/* ===== v52 premium itinerary timeline rendering only ===== */
/* 只覆寫每日行程清單的 HTML 渲染；不更動 CRUD 函數、不更動資料結構 */
...
/* ===== v53 remove timeline map pin icon only ===== */
/* 只移除行程標題前的 📍 視覺圖示，不動資料結構與 CRUD 邏輯 */
function premiumPlanCard(p){
  const t = itineraryTimeText(p);
  return `<div class="itineraryItem" data-id="${p.id}">
    <div class="itineraryDotWrap"><span class="itineraryDot"></span></div>
...
/* ===== v54 optional address field for itinerary + OSM priority search ===== */
/*
資料結構最小擴充：
...
/* ===== v55 pause map UI, keep address field and future map flexibility ===== */
/*
保留：
- plan.address 選填欄位
...
/* ===== v56 photo book only refinements: clearer cover subtitle + photo add flow + editable journal ===== */
...
/* ===== v58 PDF-only travel book engine ===== */
/* 不改照片書資料，只在匯出 PDF 時產生 print-only A4 DOM */
...
/* ===== v59 split PDF export to standalone export.html ===== */
...
/* ===== v60 index: open publisher page, do not auto-print ===== */
function openPdfPublisher(autoPrint=false){
  try{ save(); }catch(e){}
...
/* ===== v63 login whitelist + multi-trip data layer ===== */
...
/* ===== v63.2 cloud sync reliability patch =====
   目的：修正 v63/v63.1 在多旅程架構下，部分儲存只留在 localStorage、未穩定寫入 Firestore 的問題。
   原則：不改 data 內容結構，只強化 uid + tripId 的讀寫路徑、雲端狀態與錯誤提示。
*/
...
/* ===== v63.4 UI refinement: move theme settings into clear modal + larger tabs ===== */
...
/* ===== v63.5 theme settings actual relocation fix ===== */
...
/* ===== v63.6 trip card management + per-trip color UI only =====
   目的：
   - 我的旅程卡片新增「刪除」功能
   - 移除旅程卡片固定 J 字樣，改為多人使用友善設計
   - 每趟旅程可選 5 種卡片背景色，僅寫入 tripIndex 的 cardColor，不改旅程 data 結構
*/
...
/* ===== v63.7 footer + edit modal + trip color UI cleanup =====
   - 不更動旅程資料主結構 / Firebase 路徑 / Cloudinary / 行程與預算功能
   - 新增全站固定頁尾：登入頁、旅程清單、主功能頁都會顯示
   - 標題與副標改成自製彈窗，不再使用瀏覽器 prompt
   - 旅程卡片色系只保留圓點選擇，移除下拉選單
*/
...
/* ===== v64 AI assistant integration + pocket spot exploration only ===== */
...
/* ===== v64.1 AI health check refinement only =====
   - AI 健檢只使用已排入行程，不把口袋景點列入健檢資料
   - 新增非必填旅遊偏好 / MBTI / 星座，僅用於 prompt 參考
   - 行程健檢匯入後顯示在行程頁的「AI 健檢建議」卡片，不再無感
   - 修正口袋景點探索內 AI 摘要彈窗層級
*/
...
/* ===== v64.2 AI brand visual restore layer =====
   目的：恢復 Janeselect 品牌主視覺，不動 data / localStorage / Firebase / Cloudinary / AI 匯入結構。
   範圍：Logo、favicon、登入頁、旅程清單頁、主頁 badge、頁尾品牌文字。
*/
...
/* ===== v64.3 date adjustment protection layer =====
   目的：救援已輸入資料。修改旅遊日期不再清空行程、住宿、預算、照片書與行李。
   範圍：覆寫日期變更流程，提供「增減天數」與「整趟平移」。不改 Firebase / localStorage key / Cloudinary / 主資料結構。
*/
...
/* ===== v64.4 AI spot prompt enhancement + compact AI bars =====
   目的：AI 找景點加入航班、住宿、已排行程與偏好參考，讓備選日期/時間更準。
   範圍：只調整 AI 提示詞、AI 工具列與 AI 匯入口袋景點備註，不動 Firebase/localStorage key/Cloudinary/旅程主資料結構。
*/
...
/* ===== v64.5 AI spot suggested time field fix =====
   目的：口袋景點本身就有候選日期、預設開始、預設結束；AI 匯入時直接寫入 spot.day/start/end，排入行程時再帶入行程表單。
   範圍：只擴充口袋景點的選填 start/end 欄位與 AI 匯入，不改 Firebase/localStorage key/Cloudinary/旅程主資料結構。
*/
...
/* ===== v64.6 flight direct + photo book cover/photo UI fixes ===== */
...
/* ===== v64.7 photo diary upload + sync queue fix ===== */
...
/* ===== v64.8 account sync status UI fix only ===== */
...
/* ===== v64.9 single editor device + overwrite protection =====
   目的：解決同一帳號於手機/平板同時開啟同一旅程時，舊資料自動同步覆蓋新資料的問題。
   原則：不改 data 主結構、不拆 Firestore、不改 Cloudinary；只在旅程文件加 editingLock 頂層欄位。
*/
...
/* ===== v65.0 photo book cover + photo diary flow ===== */
...
function v653ShareHtml(){
  const title=v653Clean(data?.meta?.title)||'我的旅行行程';
  const dest=v653Clean(data?.trip?.dest)||'未設定目的地';
  const totalPlans=v653AllPlans().length;
  const hotelCount=(data?.hotels||[]).length;
  const flightCount=v653FlightSegments('out').length+v653FlightSegments('back').length;
  ...
  return `<div class="v653ShareBox" role="dialog" aria-modal="true" aria-label="分享簡易行程">
    <div class="v653ShareHero">
      ...
      <div class="v653ShareMetaGrid">
        <div class="v653ShareMetaCard"><span>航班段數</span><b>${flightCount || '未填'}</b></div>
        <div class="v653ShareMetaCard"><span>住宿筆數</span><b>${hotelCount || '未填'}</b></div>
        <div class="v653ShareMetaCard"><span>正式行程</span><b>${totalPlans} 個</b></div>
      </div>
    </div>
...
function v653PrintableHtml(){
  ...
  const flightCount=v653FlightSegments('out').length+v653FlightSegments('back').length;
  const hotelCount=(data?.hotels||[]).length;
  const planCount=v653AllPlans().length;
  ...
  return `<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${v653Html(title)}</title></head><body><div class="page"><div class="hero"><div class="brand">貞選旅管家 Janeselect Travel Manager</div><h1>${v653Html(title)}</h1><div class="sub">${v653Html(data?.trip?.dest||'')}｜${v653Html(v653TripRange())}</div><div class="meta"><div><span>航班段數</span><b>${flightCount||'未填'}</b></div><div><span>住宿筆數</span><b>${hotelCount||'未填'}</b></div><div><span>正式行程</span><b>${planCount} 個</b></div></div></div><div class="content"><div class="sec"><h2>✈️ 機票與交通</h2>${v653FlightHtml('out')}${v653FlightHtml('back')}</div><div class="sec"><h2>🏨 住宿資訊</h2>${v653HotelHtml()}</div><div class="sec"><h2>🗓️ 每日簡易行程</h2>${dayPrint}</div></div><div class="footer">貞選旅管家 Janeselect Travel Manager</div></div><script>setTimeout(()=>window.print(),350)<\/script></body></html>`;
}
...
function v654OpenItineraryShare(){
  const modal=v653EnsureShareModal();
  modal.innerHTML=v654ShareHtml();
  modal.classList.add('show');
}

function v654PrintableHtml(){
  const title=v653Clean(data?.meta?.title)||'簡易行程';
  const dayPrint=(data.days||[]).map(d=>{
    const plans=sortedPlans(d.key).filter(p=>!(p.source==='flight'||p.source==='hotel'));
    return `<div class="item"><div class="main">${v653Html(d.title)}｜${v653Html(d.label)}</div>${plans.length?plans.map(p=>`<div class="plan"><span class="pill">${v653Html([p.start,p.end].filter(Boolean).join('－')||'未定')}</span><div><div class="main">${v653PlanIcon(p.type)} ${v653Html(p.name||'未命名行程')}</div>${p.address?`<div class="mini">地址：${v653Html(p.address)}</div>`:''}${p.note?`<div class="mini">注意：${v653Html(p.note)}</div>`:''}</div></div>`).join(''):`<div class="mini">尚未安排正式行程</div>`}</div>`;
  }).join('');
  return `<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${v653Html(title)}</title></head><body><div class="page"><div class="hero"><div class="brand">貞選旅管家 Janeselect Travel Manager</div><h1>${v653Html(title)}</h1><div class="sub">${v653Html(data?.trip?.dest||'')}｜${v653Html(v653TripRange())}</div></div><div class="content"><div class="sec"><h2>✈️ 機票與交通</h2>${v653FlightHtml('out')}${v653FlightHtml('back')}</div><div class="sec"><h2>🏨 住宿資訊</h2>${v653HotelHtml()}</div><div class="sec"><h2>🗓️ 每日簡易行程</h2>${dayPrint}</div></div><div class="footer">貞選旅管家 Janeselect Travel Manager</div></div><script>setTimeout(()=>window.print(),350)<\/script></body></html>`;
}

function v654PrintItinerary(){
  const w=window.open('', '_blank');
  if(!w) return alert('請允許瀏覽器開啟新視窗後再試一次。');
  w.document.open();
  w.document.write(v654PrintableHtml());
  w.document.close();
}

function v654UpdateFooterVersion(){
  document.querySelectorAll('footer strong,.siteFooterVersion').forEach(el=>el.textContent=V654_VERSION_SHORT);
}
...
window.v653ShareHtml = v654ShareHtml;
window.v653OpenItineraryShare = v654OpenItineraryShare;
window.v651OpenItineraryShare = v654OpenItineraryShare;
window.v653PrintableHtml = v654PrintableHtml;
window.v653PrintItinerary = v654PrintItinerary;
window.v651PrintItinerary = v654PrintItinerary;
window.v653UpdateFooterVersion = v654UpdateFooterVersion;
window.v653UpdateHelpLog = v654UpdateHelpLog;

setTimeout(()=>{try{v654UpdateFooterVersion(); v653InsertPlannerShareButton(); v654UpdateHelpLog();}catch(e){console.warn(e)}},1040);
setTimeout(()=>{try{v654UpdateFooterVersion(); v653InsertPlannerShareButton(); v654UpdateHelpLog();}catch(e){console.warn(e)}},1450);
